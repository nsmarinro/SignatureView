...
open class SignatureView : View {
    ...
    private fun getImageFromBitmap(bitmap: Bitmap): Mat {
        ...
        org.opencv.android.Utils
            .bitmapToMat(bmp32, img)

        return img
    }

    private fun getRoiSignature(
        inputFrame: Mat,
        roi: org.opencv.core.Rect,
    ): Mat = Mat(inputFrame, roi)
    ...
    open fun isSignature(): Boolean = mPath?.isEmpty?.not() ?: false
    ...
    private fun setBackgroundToTransparent(bitmap: Bitmap): Bitmap {
        ...
        for (i in 0 until height * width) {
            if (arrayResult[i] == Color.WHITE) {
                arrayResult[i] = Color.TRANSPARENT
            } else {
                arrayResult[i] = mPaint?.color!!
            }
        }
    ...
    fun getSignatureBitmap(drawBoundingBox: Boolean = false): Bitmap? {
        if (isSignature()) {
            val isInitOpenCv: Boolean = OpenCVLoader.initLocal()
            Log.i("OpenCV", "OpenCV initLocal = $isInitOpenCv")
            ...
            if (drawBoundingBox) {
                Imgproc.drawContours(
                    ...
                    contours,
                    -1,
                    org.opencv.core.Scalar(255.0, 255.0, 0.0),
                    3,
                )
            }
            ...
            if (mWidth != null && signatureImage.width().toDouble() > mWidth!!) {
                Imgproc.resize(
                    signatureImage,
                    signatureImage,
            ...
            org.opencv.android.Utils
                .matToBitmap(signatureImage, bitOutput)
